        ...
        <div className="flex flex-row gap-2 sm:gap-3 justify-center items-center pt-4 sm:pt-6">
          {isConnected ? (
            ...
            <div className="text-center space-y-2 sm:space-y-4">
              <p className="text-xs sm:text-sm md:text-base text-gray-400 px-2">Connect your wallet to start playing</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl sm:rounded-3xl overflow-hidden border-2 border-white/20 shadow-2xl max-w-2xl mx-auto">
        ...
        >
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            {slides[currentSlide] && (
              <>
                <div className={`w-6 h-6 sm:w-8 sm:h-8 bg-white/5 rounded-md flex items-center justify-center border-[0.5px] border-white/10 flex-shrink-0`}>
                  {(() => {
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm sm:text-base md:text-lg font-semibold text-white truncate">{slides[currentSlide].title}</h3>
                  <p className="text-xs sm:text-sm text-gray-400 line-clamp-1 hidden sm:block">{slides[currentSlide].description}</p>
                </div>
              </>
            )}
          </div>
        ...
        >
          <div className="p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-orange-500 mb-3 sm:mb-4 text-center">How It Works</h2>
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-xs sm:text-sm font-semibold text-white mb-0.5 sm:mb-1">{item.title}</h3>
                    <p className="text-[10px] sm:text-xs text-gray-300 hidden sm:block">{item.desc}</p>
                  </div>
                );
              })}
            </div>
